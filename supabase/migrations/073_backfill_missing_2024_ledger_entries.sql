...
-- Each INSERT is guarded by `WHERE EXISTS (... source record's own id)` — the
-- hardcoded company_id/warehouse_id/material/reference ids below only exist
-- in the hosted production database this repair targeted. On a fresh
-- database (e.g. the standalone desktop build's own empty embedded
-- Postgres), the source dispatch/bill doesn't exist, so each guard makes its
-- INSERT a no-op instead of an FK violation on stock_ledger.
...
SELECT
  ...
  '2024-04-11',
  'HR0324-0001', NULL
WHERE EXISTS (SELECT 1 FROM dispatch_orders WHERE id = 'aeec9ce2-9393-4825-8cb0-1c6248912b84');
...
SELECT
  ...
  '2024-04-04',
  'CR0324-0009', NULL
WHERE EXISTS (SELECT 1 FROM dispatch_orders WHERE id = '1f9443df-284b-4002-a4ce-b3dd6c2dcfd7');
...
SELECT
  ...
  '2024-07-12',
  'CR0724-0053', NULL
WHERE EXISTS (SELECT 1 FROM purchase_bills WHERE id = '02eb2124-5d70-4f66-a4a8-8b3e55886a17');
