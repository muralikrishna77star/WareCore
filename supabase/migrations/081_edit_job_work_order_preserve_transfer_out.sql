-- ============================================================
-- WareCore WMS - edit_job_work_order() must not erase an
-- already-completed vendor-to-vendor transfer
--
-- Found while investigating why the vendor-stock fix applied to
-- JW-MSA5JKMM-FEG0 (migration 078) reappeared: re-saving that order via
-- Edit Order silently deleted its JOB_WORK_TRANSFER_OUT ledger row and
-- reset job_work_items.quantity_transferred_out back to 0, reintroducing
-- the double-counted vendor stock (v_stock_at_vendors summing the same
-- material twice — once still "outstanding" on the source order, once on
-- the transfer-destination order).
--
-- Root cause, in the function as of migration 069:
-- 1. The stock_ledger DELETE for the edited order only carved out
--    vendor-direct-sale virtual-return rows — JOB_WORK_TRANSFER_OUT wasn't
--    excluded, so any transfer-out ever recorded against this order was
--    wiped by an unrelated edit (a changed date, a note, anything).
-- 2. job_work_items is dropped and rebuilt from scratch each save. The
--    function already preserves is_transfer_line/source_job_work_item_id
--    per purchase_line_id (migration 067) for a line that's a transfer
--    *destination*, but never captured or restored quantity_transferred_out
--    for a line that's a transfer *source* — it silently defaulted to 0.
--
-- Fix: capture quantity_transferred_out alongside the existing
-- is_transfer_line/source_job_work_item_id snapshot, restore it on the
-- rebuilt line, and stop deleting JOB_WORK_TRANSFER_OUT rows (mirroring the
-- vendor-direct-sale carve-out already in place, and consistent with
-- migration 061's reasoning for excluding JOB_WORK_TRANSFER_OUT from
