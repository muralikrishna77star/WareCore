-- ============================================================
-- WareCore WMS - Per-item received date for Job Work output materials
--
-- Same class of problem the input side had (migration 065): fn_job_work_
-- output_item_to_ledger() hardcodes entry_date = CURRENT_DATE for every
-- JOB_WORK_OUTPUT_IN posting. One input Job Line ID can fan out into
-- several output rows of different size/category (already supported via
-- source_job_line_id), and those sub-items can genuinely arrive from the
-- vendor on different days — a single shared date is as wrong here as it
-- was for input returns.
--
-- Fix: add job_work_output_items.received_date (nullable DATE, same
-- pattern as job_work_items.received_date from migration 065), have the
-- trigger use COALESCE(NEW.received_date, CURRENT_DATE), and thread it
-- through edit_job_work_order()'s output-item insert loop.
-- ============================================================

ALTER TABLE job_work_output_items ADD COLUMN IF NOT EXISTS received_date DATE;
...
BEGIN
  ...
  IF NEW.material_type_id IS NOT NULL THEN
    ...
    INSERT INTO stock_ledger (
      ...
      COALESCE(NEW.received_date, CURRENT_DATE), v_order.created_by,
...
-- Thread received_date through edit_job_work_order()'s output-item insert
-- (everything else identical to migration 067's version).
