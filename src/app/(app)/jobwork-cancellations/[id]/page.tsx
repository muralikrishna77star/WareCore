...
export default async function JobWorkCancellationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const items: JobWorkCancellationInputItem[] = record.job_work_cancellation_items ?? []
  const outputItems: JobWorkCancellationOutputItem[] = record.job_work_cancellation_output_items ?? []

  return (
    <div className="w-full max-w-[1920px] mx-auto">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div>
          <Link href="/jobwork-cancellations" className="inline-flex items-center gap-1 text-sm text-blue-600 hover:underline mb-1">
            <ArrowLeft className="h-4 w-4" /> Job Work Cancellations
          </Link>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 sticky top-0 z-10">
              <tr>
                ...
                <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Qty Sent</th>
                <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Qty Received</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {items.length === 0 ? (
                <tr><td colSpan={9} className="px-3 py-8 text-center text-gray-400">No line items.</td></tr>
              ) : items.map((item, idx) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2 text-[0.8125rem] text-gray-700 text-right font-mono">{Number(item.quantity_received ?? 0).toFixed(3)}</td>
              ...
              <tbody className="divide-y divide-gray-100">
                {outputItems.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-[0.8125rem] text-gray-600">{item.notes || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Link href="/jobwork-cancellations"
        ...
        <ArrowLeft className="h-4 w-4" /> Back to Cancellations
      </Link>
    </div>
  )
}
