...
export default function PurgeDispatchButton({ orderId }: { orderId: string }) {
  ...
  const handlePurge = async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/dispatch/${orderId}/purge`, { method: 'POST' })
      ...
      router.push('/sale-cancellations')
      router.refresh()
    } catch {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            <h2 className="text-base font-semibold text-gray-900">Purge Cancelled Sale</h2>
            <p className="text-sm text-gray-500 mt-1">
              This will permanently archive this cancelled sale record. It will be moved to Sale Cancellations and removed from the dispatch list.
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-3 justify-end">
          ...
          >
            ...
            {loading ? 'Purging…' : 'Yes, Purge Record'}
          </button>
        </div>
      </div>
    </div>
